...
export default function Sidebar({ isOpen, closeSidebar }) {
  ...
  const [profileData, setProfileData] = useState({ isVerified: false, isPro: false, avatarUrl: null });
  ...
  const fetchProfile = async (userId) => {
    const { data } = await supabase
      .from('user_profiles')
      .select('id_verified, is_pro, avatar_url')
      .eq('id', userId)
      .single();
    if (data) {
      setProfileData({ isVerified: data.id_verified, isPro: data.is_pro, avatarUrl: data.avatar_url });
    }
  };
  ...
  useEffect(() => {
    if (!user) return;
    const profileSub = supabase.channel('sidebar_profile_sync')
      .on(
        'postgres_changes', 
  ...
  // ⚡ THE ROUTING ENGINE: ניווט פרוגרמטי נקי
  ...
  const getLinkStyle = (path) => {
    ...
    return `nav-link block w-full text-center py-2.5 px-3 rounded-lg font-medium transition-all duration-200 ${
      isActive 
        ? 'bg-fi-accent text-black shadow-[0_0_15px_rgba(255,215,0,0.3)]' 
    ...
    >
      ...
      {user && (
        <div
          ...
          className="px-4 py-6 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors group"
          id="user-profile-btn"
          role="button"
          tabIndex={0}
        >
          <div className="flex flex-col items-center text-center pointer-events-none">
            <div className="relative inline-block mb-2 transition-transform duration-300 group-hover:scale-105">
              {/* ⚡ THE REVERT: חזרה מדויקת ל-Felix המקורי */}
              <img
                id="display-avatar"
                src={profileData.avatarUrl || "https://api.dicebear.com/7.x/avataaars/svg?seed=Felix"}
                alt="Profile"
                className="w-16 h-16 rounded-full object-cover bg-fi-accent display-avatar shadow-lg shadow-fi-accent/10"
              />
            </div>
            <div className="flex items-center justify-center gap-1.5 flex-wrap">
              <span id="display-username" className="font-bold text-white truncate max-w-[140px] tracking-wide group-hover:text-fi-accent transition-colors duration-200">
          ...
          <div className="mt-4 w-full flex justify-center pointer-events-none">
            <TrustBadge 
              username={user.user_metadata?.user_name || user.email?.split('@')[0]} 
              className="bg-black/80 border border-white/10 px-4 py-2 rounded-full text-sm shadow-inner group-hover:border-fi-accent/30 transition-colors duration-200" 
            />
          </div>
        </div>
      )}
      ...
      <nav className="mt-4 px-4 pb-6 space-y-1.5 text-sm text-gray-400">
        <Link to="/dashboard" onClick={closeSidebar} className={getLinkStyle('/dashboard')}>Dashboard</Link>
        <Link to="/community" onClick={closeSidebar} className={getLinkStyle('/community')}>Community</Link>
        <Link to="/my-posts" onClick={closeSidebar} className={getLinkStyle('/my-posts')}>My Posts</Link>
        <Link to="/closet" onClick={closeSidebar} className={getLinkStyle('/closet')}>My Closet</Link>
        <Link to="/trades" onClick={closeSidebar} className={getLinkStyle('/trades')}>My Trades</Link>
        <Link to="/messages" onClick={closeSidebar} className={getLinkStyle('/messages')}>Messages</Link>
        <Link to="/settings" onClick={closeSidebar} className={getLinkStyle('/settings')}>Settings</Link>
...
{/* שים לב: המודאל יושב מחוץ להתניה כדי שלא יושמד בפתאומיות כשהסטטוס משתנה */}
<UpgradeModal 
